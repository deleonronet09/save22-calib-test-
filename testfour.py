...
for word in sentence:
  dictWord[word]=[sentence.count(word)]

for sort in sorted(dictWord.items(), key=lambda val:val[1], reverse=True):
  print sort

#sentence.strip() removes whitespaces and special chars between words of the sentences of the variable sentence. lower() method converts all cases to lower case for it to be equal to words that is in uppercase. but well, i did not use strip but instead i used split method.